...
class NavigateMaze:
  ...
  def movingForward(vel_msg, velocity_publisher, t0, current_distance, distance, speed, forwardSpeed, front):  
    ...
    if (front < 1.0):
      NavigateMaze.stop(vel_msg, velocity_publisher)
    time.sleep(1) # stop for 1 second
     ...
     while not rospy.is_shutdown():
       ...
       if no_right_wall == True:
         if front < 0.5:
          ...
          #than 1.0  
          NavigateMaze.movingBackward(vel_msg, velocity_publisher, t0, current_distance, distance[1], speed, -0.36, front)  
      
          print('Turn clockwise because no right wall') #display message      
          #Turn 90 degree clockwise
      
          NavigateMaze.turnCW(vel_msg, velocity_publisher, t0, 0, 3, 90) 
          print('Move forward because no right wall')
         elif front < 1.0:
          NavigateMaze.movingForward(vel_msg, velocity_publisher, t0, current_distance, distance[1], speed, 0.36, front)
         else:
          #Move forward with distance of 0.29 when distance from front  
          #wall is greater than 0.5
          NavigateMaze.movingForward(vel_msg, velocity_publisher, t0, current_distance, distance[2], speed, 0.36, front)
       ...
       if (no_front_wall == True) and (front > 1.0):
          if (front < 0.5):
            ...
            NavigateMaze.movingForward(vel_msg, velocity_publisher, t0, current_distance,distance[0], speed, 0.36, front)
        ...
        #right wall are detected
         ...
         if (left > 0.5) and ((right > 3.0) or (top_right > 3.0)):
            ...
            if front < 0.5:
             ...
             #move backward if the distance in front direction is smaller  
             #than 1.0
             NavigateMaze.movingBackward(vel_msg, velocity_publisher,  
             ...
             NavigateMaze.turnCCW(vel_msg, velocity_publisher, t0, 0, 3, 90)

     # The node will stop when press control + C
     rospy.spin()

if __name__ == '__main__':
    client_instance = NavigateMaze()
    try:
